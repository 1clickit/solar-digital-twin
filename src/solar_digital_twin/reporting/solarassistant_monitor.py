...
<script>
let token='';const statusElement=document.getElementById('status');const esc=v=>String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));const shown=v=>v===null||v===undefined||v===''?'Not reported':esc(v);const title=s=>s.replaceAll('_',' ').replace(/\b\w/g,c=>c.toUpperCase());
...
async function update(){try{let r=await fetch('/api/status',{cache:'no-store'});render(await r.json())}catch(e){statusElement.textContent='Unknown'}}setInterval(update,2000);update();
